locals {
  ...
  env-specific-supernet = cidrsubnet(local.sps-infra-supernet, 8, index(["sbox", "dev", "test", "ithc", "demo", "stg", "prod"], var.env))
}
    ...
    rt = {
      ...
      routes = {
        default = {
          ...
          next_hop_in_ip_address = var.networking.next_hop
        }
      }
    }
    appgw-rt = {
